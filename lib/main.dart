...
class _GameScreenState extends State<GameScreen> {
  ...
  void _handleSpin() {
    ...
    // Сначала голос "Вращайте барабан!"
    _playYakubovichSpinVoice();

    // А через 1.5 секунды начинаем вращение и звук барабана
  ...
  void _onLetterPressed(String letter) {
    ...
    // Больше нельзя угадывать буквы, пока не закончим обработку текущей
    _canGuessLetter = false;

    if (hits > 0) {
      final gained = hits * _pointsPerLetter;

      // Сначала звучит "Откройте!"
      _playOpenVoice();

      // Через 1.5 секунды открываем букву на экране и проигрываем звук правильной буквы
      Future.delayed(const Duration(milliseconds: 1500), () {
        if (!mounted) return;
        ...
        _playCorrectLetterSound();
        setState(() {
          _pointsPerLetter = 0;
        });
      });
    } else {
      // Неверная буква — сразу звук ошибки и переход хода
  ...
  Future<void> _showLetterSelectionDialog() async {
    ...
    if (selected != null) {
      ...
      if (hits > 0) {
        // "Откройте!" + через 1.5 секунды звук правильной буквы и открытие на экране
        _playOpenVoice();
        Future.delayed(const Duration(milliseconds: 1500), () {
          if (!mounted) return;
          _playCorrectLetterSound();
  ...
  Future<void> _startSpinSound() async {
    ...
    try {
      ...
      await _spinPlayer.setSourceAsset('sounds/wheel_spin_1995.mp3');
  ...
  Future<void> _playEffect(String asset) async {
    ...
    try {
      ...
      await _effectPlayer.setSourceAsset('sounds/$asset');
      await _effectPlayer.resume();
    } catch (_) {}
  }

  Future<void> _playWrongLetterSound() => _playEffect('wrong_letter.mp3');
  Future<void> _playCorrectLetterSound() => _playEffect('correct_letter.mp3');
  Future<void> _playYakubovichSpinVoice() => _playEffect('yakubovich_spin.mp3');
  Future<void> _playOpenVoice() => _playEffect('yakubovich_open.mp3');
  Future<void> _playPrizeSectorSound() => _playEffect('prize_sector.mp3');
  Future<void> _playBankruptSound() => _playEffect('bankrupt.mp3');
  Future<void> _playWinnerFanfare() => _playEffect('winner_fanfare.mp3');
