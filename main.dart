...
class _GameScreenState extends State<GameScreen> {
  ...
  Future<void> _playYakubovichSpinSound() => _playEffect('yakubovich_spin.mp3');
  ...
  Future<void> _playOpenThenCorrect() async {
    if (!_soundEnabled) return;
    try {
      await _effectPlayer.stop();
      await _effectPlayer.setSourceAsset('sounds/yakubovich_open.mp3');
      await _effectPlayer.resume();
      await Future.delayed(const Duration(milliseconds: 1500));
      if (!_soundEnabled) return;
      await _effectPlayer.stop();
      await _effectPlayer.setSourceAsset('sounds/correct_letter.mp3');
      await _effectPlayer.resume();
    } catch (_) {}
  }
